    ...
    public class AccountController : BaseController
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (Request.Form["hdnTimeStamp"] != null)
                {

                    int miniouts = 0;
                    int.TryParse(Request.Form["hdnTimeStamp"].ToString(), out miniouts);
                    TimeDiff = new TimeSpan(0, miniouts, 0);
                }
                var user = await UserManager.FindAsync(model.UserName, model.Password);
                if (user != null)
                {
                    UserEntity Userobj = UserServices.GetUserByName(model.UserName);
                    if (Userobj.Status == false)
                    {
                        ModelState.AddModelError("", "Your account is Inactive.");
                        return View(model);
                    }
                    HRMHelper.CurrentUser = Userobj;
                    HRMHelper.TimeDiffrence = TimeDiff;
        ...
        public ActionResult Register()
        {
            ...
            var Roleslist = context.Roles.OrderBy(r => r.Name).ToList().Select(rr => new SelectListItem { Value = rr.Name.ToString(), Text = rr.Name }).ToList();
        ...
        {
            List<UserEntity> model = new List<UserEntity>();
            model = UserServices.GetAllUser();
            return View(model);
        ...
        [Authorize]
        [OutputCache(Duration = 2, Location = OutputCacheLocation.Client, VaryByParam = "id")]
        public async Task<ActionResult> EditUser(int? Id)
        {
            if (!User.IsInRole("Admin") || (Id == null))
            {
                Id = HRMHelper.CurrentUser.UserId;
            }
            List<UserEntity> RoleManagers = UserServices.GetUserByRoleName("Manager");
            List<UserEntity> RoleAdmins = UserServices.GetUserByRoleName("Admin");
            RoleManagers.AddRange(RoleAdmins);
            ...
            if ((Id != null) && (Id > 0))
            {
                ...
                UserEntity UserObj = UserServices.GetUserByID(UserId);
                if (UserObj != null)
                {
                    model.UserId = UserObj.UserId;
                    model.UserName = UserObj.UserName;
                    model.Name = UserObj.Name;
                    model.Email = UserObj.Email;
                    model.PersonalEmail = UserObj.PersonalEmail;
                    model.ContactNo = UserObj.ContactNo;
                    model.Address = UserObj.Address;
                    model.Salary = UserObj.Salary;
                    model.ManagerId = UserObj.ManagerId;
                    model.DesignationId = UserObj.DesignationId;
                    model.DOB = UserObj.DOB;
                    model.DOJ = UserObj.DOJ;
                    model.DOR = UserObj.DOR;
                    model.Status = (bool)UserObj.Status;
                    model.Image = UserObj.Image == null ? string.Empty : UserObj.Image;
                    var _user = await UserManager.FindByNameAsync(UserObj.UserName);
                    IList<string> UserRoles = await UserManager.GetRolesAsync(_user.Id);
